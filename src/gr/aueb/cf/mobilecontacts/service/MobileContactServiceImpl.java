    ...
    public MobileContact updateMobileContact(MobileContactUpdateDTO dto) throws PhoneNumberAlreadyExistsException, ContactNotFoundException {
        ...
        try {
            if (!dao.userIDExists(dto.getId())) {
                throw new ContactNotFoundException("Error contact with id: " + dto.getId() + " not found for update");
            }
            ...
            if (isPhoneNumberExists && !isPhoneNumberOurOwn) {
                throw new PhoneNumberAlreadyExistsException("Contact with phone number: " + dto.getPhoneNumber() + " already exist and can not be updated.");
            }
